...
public class ZZPath {
    ...
    static final String drtext(ZZCell c) {
        return drdim(c);
    }
    ...
    static public ZZPath createFromStructure1(ZZCell c) {
        ...
        while (c!=null) {
            ...
            String s = c.getText();
            if (s.equals("STEP")) {
                if (args.length < 2) throw new ZZError("Not enough args for '" + s + "'");
                p = p.step(drdim(args[0]), drdir(args[1]));
            } else if (s.equals("HEAD") || s.equals("END")) {
                if (args.length < 2) throw new ZZError("Not enough args for '" + s + "'");
                p = p.headcell(drdim(args[0]), drdir(args[1]));
            } else if (s.equals("FIND")) {
                if (args.length < 3) throw new ZZError("Not enough args for '" + s + "'");
                p = p.untilstring(drdim(args[0]), drdir(args[1]), drtext(args[2]));
            } else {
    ...
    public ZZCell readFrom(ZZCell from, boolean error, ZZObs o) {
        if (from==null) throw new ZZError("Can't read path from null!");
        ZZCell n = from;
    ...
    ZZPath headcell(final String dim, final int dir, final boolean ensuremove) {
        return appendOp(new PathOp() {
            public ZZCell apply(ZZCell from, ZZObs o) {
                return from.h(dim, dir, ensuremove, o);
            }

            public String toString() {
                return "headcell(" + dim + "," + dir + "," + ensuremove + ")";
            }
        });
    }
