 ...
 */
abstract public class ZZViewComponent extends Panel implements ZZView, MouseListener, MouseMotionListener {
    ...
    static void redoCtrls(ZZCell vc) {
        ...
        ZZCell cc, dc;
        if ((cc = vc.s("d.ctrlview", -1))!=null) dc = vc;
        else if ((dc = vc.s("d.ctrlview", 1))!=null) cc = vc;
        else cc = (dc = vc);
...
*/
    ...
    protected ZZViewComponent(ZZCell viewCell0) {
        this();
    ...
    protected void processKeyEvent(KeyEvent e) {
        ...
        if (id==e.KEY_PRESSED) {
            ...
            // appear. Very odd.
            if (kc==e.VK_SHIFT || kc==e.VK_CONTROL || kc==e.VK_ALT || kc==e.VK_META) {
            ...
            try {
                e.consume();
                if (kbd!=null) kb.perform(e, kbd.dataView, kbd.ctrlView, null);
                else kb.perform(e, dataView, ctrlView, null);
    ...
    public void paint(Graphics gr) {
        if (dblbuf) {
            ...
            Dimension d = getSize();
            if (cache==null || cacheSize==null || !cacheSize.equals(d)) {
                cacheSize = d;
                p("Creating cache: " + d.width + " " + d.height);
                cache = null;
                if (d.width > 0 && d.height > 0) cache = createImage(d.width, d.height);
            }
            if (cache==null) {
                paintInto(gr);
            } else {
