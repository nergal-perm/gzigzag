...
public class FirstMatchBreaker {
    ...
    static class Word {
        ...
        FText.Part get() {
            return p.subpart(start, end);
        }

        int width() {
            return p.fm.stringWidth(p.s.substring(start, end));
        }
    ...
    public static Vector prepare(FText txt) {
        ...
        if (txt.parts.length==0) throw new ZZError("FText has length 0");
        Vector v = new Vector();
