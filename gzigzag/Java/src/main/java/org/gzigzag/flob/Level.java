...
import org.gzigzag.*;
import org.gzigzag.flob.LevelView;
...
public class Level {
    ...
    static class DimSet {
        ...
        DimSet(ZZCell viewc) {
            ...
            linkdim = new DimVector(ZZCursorReal.get(dimcurs[0]).getText(), 1);
            targetdim = new DimVector(ZZCursorReal.get(dimcurs[1]).getText(), 1);
            tunneldim = new DimVector(ZZCursorReal.get(dimcurs[2]).getText(), 1);
            rev = new DimSet(this);
        }
    ...
    static class Performer {
        ...
        Performer(ZZCell viewc) {
            this.viewc = viewc;
            c = ZZCursorReal.get(viewc);
            ds = new DimSet(viewc);
            lnk = ds.islink(c);
            // XXX why doesn't this work:
            // act = ZZDefaultSpace.getDirActionWaitingAndClear(viewc);
            ZZCell nac = ZZDefaultSpace.findOnClientlist(c.getSpace(), "NextAction", true);
            nac = nac.getOrNewCell("d.1", 1, null);
            act = nac.getText();
            nac.setText("");
        }

        public void perf(ZZCell to, DimVector dim) {
            perf(to, dim, c);
        }
        ...
        public void perf(ZZCell to, DimVector dim, ZZCell from) {
            ...
            if (act.equals("")) ZZCursorReal.set(viewc, to);
            ...
            } else System.out.println("Level module can't do " + act);
        }
        ...
        public void dir(int d, int b) {
            ...
            if (lnk) {
                if (b==0) {
                    perf(ds.targetdim.end(c), null);
                    return;
                } else if (b < 0) {
                    to = ds.targetdim.get(c);
                    if (to!=null && ds.targetdim.get(to)!=null) perf(to, ds.targetdim);
                } else {
                ...
                if (b < 0) {
                    ...
                    if (to==null && rnk.length > 0) to = rnk[rnk.length / 2];
