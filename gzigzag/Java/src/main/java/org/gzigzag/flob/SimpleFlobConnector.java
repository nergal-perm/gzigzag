...
public class SimpleFlobConnector implements FlobDecorator, ZOb {
    ...
    public void decorate(final FlobSet f, final String path, final ZZCell view) {
        ...
        if (connection==null) throw new ZZError("Must provide value for FlobConnector connection");

        final ConnectionLineDecor.Builder clb = new ConnectionLineDecor.Builder(f, null);
        clb.startl(100, 1);
        ...
        f.iterDepth(new FlobSet.DepthIter() {
            public void act(Flob[] flobs, int start, int n) {
                ...
                for (int i = start; i < start + n; i++) {
                    Flob cur = flobs[i];
                    if (cur.c==null || !cur.flobPath.equals(path)) continue;
