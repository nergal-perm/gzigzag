...
public class ZZCacheDimSpace extends ZZDimSpace {
    ...
    synchronized public int commit() {
        ...
        for (Enumeration e = getDims().keys();
             e.hasMoreElements(); ) {
            String dname = (String) e.nextElement();
            ZZDimension d = (ZZDimension) getDims().get(dname);
        ...
        ct.endCommit(coutf);
        cwrit.endWrite(true);
        for (Enumeration e = getSpaceParts().keys(); e.hasMoreElements(); ) {
            String s = (String) e.nextElement();
            ZZSpacePart sp = (ZZSpacePart) getSpaceParts().get(s);
            sp.postCommitHook();
        }
        return stmp;
    }
    ...
    public ZZCacheDimSpace(StreamSet w, int tstamp, boolean readonly) {
        ...
        getSpaceParts().put("bintree", new BinaryTreePart(this, "bintree"));
        getSpaceParts().put("imptree", new ImpliedTreePart(this, "imptree"));
        getSpaceParts().put("imptree2", new ImpliedTreePart2(this, "imptree2"));
        getSpaceParts().put("doubler", new DoubleStepPart(this, "doubler"));
        getSpaceParts().put("version", new VersionPart(this, "version"));

        getSpaceParts().put("gentreetest", new GenTreePart(this, "gentreetest",
                new GenTreeTestModel()));

        try {
            getSpaceParts().put("nile1str", new GenTreePart(this, "nile1str",
                    (GenTreePartModel)
        ...
        try {
            ...
            getSpaceParts().put("zaubertrank", sp);
