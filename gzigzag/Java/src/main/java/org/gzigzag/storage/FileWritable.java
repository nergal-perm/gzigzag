    ...
    public void write(long offset, byte[] data) {
        ...
        try {
            if (!cache) {
                ...
                // clear read cache
                if (offset < off + r_size && offset + data.length > off) {
                ...
                } else if (offset==w_off + written && BUF_SIZE - written > data.length) {
                    //p(" hit: " + offset);
    ...
    public byte[] read(long offset, int n) {
        ...
        try {
            ...
            if (!cache) {
                ...
                if (offset < w_off + written && offset + n > w_off) {
                    flush();
                }
    ...
    public void read(long offset, byte b[], int o, int n) {
        ...
        try {
            if (!cache) {
                ...
                if (offset + o < w_off + written && offset + o + n > w_off) {
                    flush();
                }

                if (offset + o > off && offset + o - off + n < r_size) {

                    System.arraycopy(r_buffer, (int) (offset + o - off), b, o, n);
                } else {
                    f.seek(offset + o);
                    r_size = f.read(r_buffer);
