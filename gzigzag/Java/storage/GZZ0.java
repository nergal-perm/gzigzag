...
public class GZZ0 {
    ...
    public int read(Runner r, int maxtimestamp) {
        ...
        try {
            ...
            i = dis.readInt();
            if (i!=version) throw new ZZFatalError("Invalid file version! Not " + version);
            i = dis.readInt();
            if (i!=r.getContentType()) throw new ZZFatalError("Wrong content type!");
            i = dis.readInt();
            if (i!=r.getContentVersion()) throw new ZZFatalError("Wrong content version!");
        } catch (IOException e) {
            ZZLogger.exc(e);
        ...
        try {
            while (true) {
                ...
                eofOk = false;
                if (rectype!=rTime)
                    throw new ZZFatalError("Invalid timestamp record! " + id + " " + (counts!=null ? counts.getCount():-1));
                ...
                if (nbytes > byt.length) byt = new byte[nbytes];

                dis.readFully(byt, 0, nbytes);
                ByteArrayInputStream bis = new ByteArrayInputStream(byt, 0, nbytes);
                r.readRun(bis);
            }
        } catch (EOFException e) {
            if (!eofOk) throw new ZZFatalError("Unexpected eof in simpledimfile");
        } catch (IOException e) {
            ZZLogger.exc(e);
            throw new ZZFatalError("Reading file");
        }
