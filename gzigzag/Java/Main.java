...
import org.gzigzag.media.SpanTree;
...
public class Main {
    ...
    public static void main(String argv[]) {
        ...
        try {
            ...
            clop.put("new", noargs, "Recreate the default client space " + "starting from the main cell. This " + "is important if the internal client " + "structures used by GZigZag have changed " + "since the space was created, or if this " + "is a new space. If the cellscroll file " + "given does not exist, this is the default. " + "If the file already exists, the old system " + "list and the list on d.2 down from the home " + "cell are placed on the d.2 list, at a cell " + "named OldSysList (see ZZDefaultSpace).");
            clop.put("import", new String[]{"XMLFILE"}, "When creating a new space, you can import an XML " + "file describing the space.");
            clop.put("export", new String[]{"XMLFILE"}, "Export the whole space to the specified file");
            clop.put("convert", noargs, "Convert an old, incompatible, space to compatible " + "form. When conversion is needed, it will be " + "told by the program. Taking backup before " + "conversion is recommended. After conversion you " + "might need to use -new and -dcold to catch up " + "with configuration changes.");
            clop.put("xmlencoding", new String[]{"ENCODING"}, "Specify the character encoding used by the " + "XML export function.");
            clop.put("ztpserv", new String[]{"PORT"}, "Invoke the ZTP server code instead of the ZZ " + "client code. Listen on the TCP port PORT for " + "incoming connections.");
            clop.put("recrmdim", noargs, "Recreate master dimension list from data on disk.");
            clop.put("foolconvert", noargs, "???");
            clop.put("textlog", noargs, "???");
            clop.put("verboselog", noargs, "???");
            clop.put("span", noargs, "Save information about cells whose " + "spans overlap. This may be slow at the moment, " + "but is necessary to show e.g. the overlaps of " + "two nile text streams.");
            clop.put("dump", noargs, "Dump the space to the file " + "CELLSCROLLFILE.dmp (where CELLSCROLLFILE is the " + "name of the space file).");
            clop.put("slurp", new String[]{"FILENAME"}, "Slurp the space from the file called FILENAME.");
            clop.put("versioneddims", noargs, "Show connections from old versions along regular " + "dimensions in addition to the special version: " + "dimensions.");
            clop.put(new CommandLineOptionsParser.Option("dbg", new String[]{"CLASSNAME"}, "Turn on debugging " + "in the given " + "class. For " + "developers	only.") {
                public void action(CommandLineOptionsParser clops, String[] args) {
                    debugClass(args[0], dbgclasses);
                }

            });
            clop.putHelp("help", "shows this help screen", true, "Usage:\nGZigZag takes one mandatory parameter: the name " + "of the directory where the space is (to be) stored. " + "Additionally, one or more of the following options " + "may be specified before the directory name.", "");
            ...
            if (textlog) {
                ZZPrimitiveActions.textlog = new PrintWriter(new FileOutputStream(argv[cur] + ".txt", true));
            }

            if (verboselog) {
                ZZLogger.logfile = new PrintWriter(new FileOutputStream(argv[cur] + ".log", true));

            }

            // *** Prepare the space: create ZZSpace objects

            System.runFinalization(); // needed for unlocking a space
            ...
            if (argv[cur].equals(":DS")) {
                ...
                if (argv.length > cur + 1) {
                    ...
                    System.arraycopy(argv, cur + 1, slices, 0, argv.length - (cur + 1));
                }
            ...
            if (slurpf!=null) {
                if (!newspace) throw new ZZError("cannot slurp into an existing space");
                ...
                for (Enumeration e = sd.stringscrolls.keys(); e.hasMoreElements(); ) {
                    ...
                    if (ss==null) throw new ZZError("cannot create the string scroll " + name);

                    if (ss.curEnd()!=0)
                        throw new ZZError("the string scroll `" + name + "' is not initially empty!");
            ...
            if (dump) {
                PrintWriter pw = new PrintWriter(new FileOutputStream(argv[cur] + ".dmp", false));
            ...
            if (ztpserv && ztpport > 0) {
                ...
                try {
                    ...
                    Constructor ztpcons = ztpcl.getConstructor(Class.forName("org.gzigzag.ZZSpace"), Class.forName("java.lang.Integer"));
                    Object ztp = ztpcons.newInstance(space, new Integer(ztpport));
            ...
            if (newspace) { // Might be re-init
                home.getOrNewCell("d.gzz-space-version", 1).setText("" + zzclientversion);
            } else {
                ...
                if (convert && !foolconvert) {
                    if (fileversion==zzclientversion) quit("File is already of same version.");
    ...
    static void convert(int fileversion, int zzclientversion, ZZSpace s) {
        try {
            ...
            while (fileversion < zzclientversion) {
                vchg = (org.gzigzag.transform.VersionChanger) (Class.forName("org.gzigzag.transform.Version" + fileversion)).newInstance();
