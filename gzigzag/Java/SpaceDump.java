...
public class SpaceDump {
    ...
    public SpaceDump(Reader reader) throws IOException {
        ...
        try {
            ...
            if (!com.name.toUpperCase().equals("H") || com.args.length!=3 || !com.args[0].toUpperCase().equals("ZZSpaceDump".toUpperCase()))
                throw new ZZError("This is not a space dump");

            if (!com.args[1].equals("0")) throw new ZZError("I don't know how to slurp this file");
        } catch (SyntaxError e) {
            throw new ZZError("This is not a space dump (syntax error: + " + e.getMessage() + ")");
        }
        ...
        while (true) {
            ...
            if (com.name.toUpperCase().equals("C")) {
                if (com.args.length!=2) throw new SyntaxError("wrong number of arguments for C");

                content.put(com.args[0], com.args[1]);
                cellswithspans.remove(com.args[0]);
            } else if (com.name.toUpperCase().equals("R")) {
                if (com.args.length < 3) throw new SyntaxError("too few arguments to R");
                ...
                if (com.args.length!=2) throw new SyntaxError("wrong number of arguments to SP");

                content.put(com.args[0], com.args[1]);
                cellswithspans.put(com.args[0], com.args[0]);
            } else if (com.name.toUpperCase().equals("SC")) {
                if (com.args.length!=2) throw new SyntaxError("wrong number of arguments to SC");
