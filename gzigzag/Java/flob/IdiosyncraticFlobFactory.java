...
public class IdiosyncraticFlobFactory implements FlobFactory, ZOb {
    ...
    public FlobFactory getCellView(ZZCell c) {
        ...
        if (cv==null) throw new ZZError("No default view!");
        return cv;
    }


    public Dimension getSize(ZZCell c, float fract) {
        if (c==null) return ((FlobFactory) def).getSize(null, fract);
    ...
    public Flob makeFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, int x, int y, int d, int w, int h) {
        return getCellView(c).makeFlob(into, c, handleCell, fract, x, y, d, w, h);
    }
    ...
    public Flob placeFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, int x, int y, int depth, float xfract, float yfract) {
        return getCellView(c).placeFlob(into, c, handleCell, fract, x, y, depth, xfract, yfract);
    }
    ...
    public Flob centerFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, Point p, int xalign, int yalign, int depth, Dimension d) {
        return getCellView(c).centerFlob(into, c, handleCell, fract, p, xalign, yalign, depth, d);
    }

    public Flob centerFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, Point p, int xalign, int yalign, int depth) {
        return getCellView(c).centerFlob(into, c, handleCell, fract, p, xalign, yalign, depth);
    }
