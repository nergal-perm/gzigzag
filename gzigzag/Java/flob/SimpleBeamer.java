/* DO NOT EDIT THIS FILE. THIS FILE WAS GENERATED FROM flob/SimpleBeamer.zob,
...
public class SimpleBeamer implements FlobDecorator {
    ...
    static public Color getSpanColor() {
        return new Color((float) (Math.random() + 1) / 2, (float) (Math.random() + 1) / 2, (float) (Math.random() + 1) / 2);
    }

    int[] xy = new int[16];

    protected void putBeam(BeamDecor.Builder bb, SpanFlob[] sfs, Span[] sps, Color c) {
        p("Putbeam");
     ...
     */
    static boolean moveBack(SpanFlob[] sfs, Span[] spans) {
        long l1 = spans[0].getStartOffs();
        long l2 = spans[1].getStartOffs();
        if (l1==l2) {
            // Check if previous spans end at same point.
            sfs[0] = sfs[0].getPrev();
            if (sfs[0]==null) return false;
            sfs[1] = sfs[1].getPrev();
            if (sfs[1]==null) return false;
            spans[0] = sfs[0].getSpan();
            if (spans[0]==null) return false;
            spans[1] = sfs[1].getSpan();
            if (spans[1]==null) return false;
    ...
    static boolean moveForw(SpanFlob[] sfs, Span[] spans) {
    ...
    public void decorate(final FlobSet into, final String path, final ZZCell view) {
            ...
            public void act(Flob[] flobs, int start, int n) {
                ...
                for (int f = start; f < start + n; f++) {
                    ...
                    for (int i = 0; i < ovs.length; i++) {
                        ...
                        if (done.get(pa)!=null) continue;
                        Color c = getSpanColor();

                        sfs[0] = sf;
                        sps[0] = sfs[0].getSpan();
                        sfs[1] = (SpanFlob) ovs[i];
                        sps[1] = sfs[1].getSpan();
