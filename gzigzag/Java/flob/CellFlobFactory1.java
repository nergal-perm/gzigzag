/* DO NOT EDIT THIS FILE. THIS FILE WAS GENERATED FROM flob/CellFlobFactory1.zob,
...
public class CellFlobFactory1 implements FlobFactory, ZOb {
    ...
    public ScalableFont font = new ScalableFont("SansSerif", Font.PLAIN, 12);
    /**
    ...
    public int[] margins = new int[]{2, 1};
    /**
        ...
        while (n!=null) {
            ...
            if (s.equals("widthstr")) {
                ...
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    if (s.equals("true")) ball = true;
                    else if (s.equals("false")) ball = false;
                    else { /* XXX ??? */ }
...
*/
    ...
    public Dimension getSize(ZZCell c, float fract) {
        ...
        if (ball) return BallCell.getSize(fm, str, margins[0], margins[1]);
        else return CellFlob1.getSize(fm, str, margins[0], margins[1]);
    }
    ...
    public Flob makeFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, int x, int y, int d, int w, int h) {
        ...
        if (ball) {
            BallCell cf = new BallCell(x, y, d, w, h, handleCell, c, c.getText(), f, fm, into);
            cf.xoffs = margins[0];
            cf.yoffs = margins[1];
            ...
            CellFlob1 cf = new CellFlob1(x, y, d, w, h, handleCell, c, c.getText(), f, fm);
            cf.bg = bg;
            cf.xoffs = margins[0];
            cf.yoffs = margins[1];
            addSolidColors(into, cf, cf);
    ...
    public Flob placeFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, int x, int y, int depth, float xfract, float yfract) {
        Dimension d = getSize(c, fract);
        return makeFlob(into, c, handleCell, fract, (int) (x - xfract * d.width), (int) (y - yfract * d.height), depth, d.width, d.height);
    }

    public Flob centerFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, Point p, int xalign, int yalign, int depth, Dimension d) {
    ...
    public Flob centerFlob(FlobSet into, ZZCell c, ZZCell handleCell, float fract, Point p, int xalign, int yalign, int depth) {
    ...
    static public void addSolidColors(FlobSet into, Flob f, Colorer col) {
        ...
        while (e.hasMoreElements()) {
            ...
            into.add(new LineDecor(x, f.y, x, f.y + f.h, Color.black, f.d));
        }

        // XXX Does not check whose mark it is - should!
        if (f.c.s("d.mark-set", 1)!=null) if (col.addColor(Color.red)) return;

        if (f.c.s("d.clone", -1)!=null) if (col.addColor(Color.yellow)) return;
        else if (f.c.s("d.clone", 1)!=null) if (col.addColor(new Color(255, 255, 192))) return;
