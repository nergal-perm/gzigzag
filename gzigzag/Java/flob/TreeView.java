/* DO NOT EDIT THIS FILE. THIS FILE WAS GENERATED FROM flob/TreeView.zob,
...
public class TreeView implements FlobView, ZOb {
    ...
    static final void pa(String s) {
        System.out.println("(flob)TreeRaster: " + s);
    }
    ...
    public float initmul = (float) 1.6;
    /**
     ...
     *
     * <p>Default value: <PRE> new int[] {3, 6};</PRE>.
     *
     * @structparam 1
     */
     ...
     * <p>Default value: <PRE> new float[] {(float) 0.7, (float) 0.8};</PRE>.
     *
     * @structparam 1
     */

    public float[] shrink = new float[]{(float) 0.7, (float) 0.8};
    /**
    ...
    /* AUTOGENERATED! */
    static final private int fullmask = 63;
        ...
        while (n!=null) {
            ...
            if (s.equals("initmul")) {
                ...
                try {
                    ...
                    gap = new int[i];
                    i = 0;
                    for (ZZCell c = n.s("d.1"); c!=null; c = c.s("d.1")) {
                        s = c.getText();
            ...
            } else if (s.equals("treelines")) {
                mask |= 16;
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    if (s.equals("true")) treelines = true;
                    else if (s.equals("false")) treelines = false;
                    else { /* XXX ??? */ }
    ...
    public class PositionDecider {
        ...
        public Vector place_tree_cells(ZZCell root) {
            ...
            if (cell!=null && !cellpos.containsKey(cell)) {
                ...
                pos.height = dcoord[currdepth] + csize[currdepth].height / 2 - s.height / 2;
            ...
            int count = 0;
            for (cell = root; cell!=null && (!cellpos.containsKey(cell) || extracells.containsKey(cell)); cell = breadth.getCellSuccessor(cell)) {
                ...
                // with us.
                if (count > canvaswidth / csize[currdepth].width && cellpos.containsKey(accursed) && cell!=accursed) {
                ...
                Dimension cpos = new Dimension((startbcoord + endbcoord - cs.width) / 2, dcoord[currdepth] + dcorr);
        ...
        public PositionDecider(DimVector breadth, DimVector depth, Dimension fs, FlobFactory fact, ZZCell accursed) {
            ...
            for (int i = maxdepth / 2 - 1; i >= 0; i--) {
                gaps[i] = new Dimension((int) (gaps[i + 1].width * shrink[0]), (int) (gaps[i + 1].height * shrink[1]));
                csize[i] = new Dimension((int) (csize[i + 1].width * shrink[0]), (int) (csize[i + 1].height * shrink[1]));
            ...
            for (int i = 0; i < maxdepth; i++) {
                smcsize[i] = new Dimension(csize[i].width / 4, csize[i].height / 4);
            }
    ...
    public void raster(FlobSet into, FlobFactory fact, ZZCell view, String[] dims, ZZCell accursed) {
        ...
        PositionDecider pd = new PositionDecider(breadth, depth, into.getSize(), fact, accursed);
        ...
        int bcorr = acpos.width - (depthhorizontal ? into.getSize().height:into.getSize().width) / 2;
        ...
        for (Enumeration e = pd.getCells(); e.hasMoreElements(); ) {
            ...
            fact.makeFlob(into, cell, cell, 1, pos.width, pos.height, d, cs.width, cs.height);

            // draw tree lines
            if (treelines) {
                for (Enumeration f = pd.getCellChildren(cell); f.hasMoreElements(); ) {
                    ...
                    if (depthhorizontal) {
                        into.add(new LineDecor(pos.width + cs.width, pos.height + cs.height / 2, cpos.width, cpos.height + cs.height / 2, Color.red, (pd.getCellDepth(child) + d) / 2));
                    } else {
                        into.add(new LineDecor(pos.width + cs.width / 2, pos.height + cs.height, cpos.width + chs.width / 2, cpos.height, Color.red, (pd.getCellDepth(child) + d) / 2));
                    }
                }
            }
        ...
        if (true) {
            ...
            Flob bfl = fact.makeFlob(into, bdim, bdim, 1, s.width, 0, 1, s.width, s.height);
            Flob dfl = fact.makeFlob(into, ddim, ddim, 1, 0, s.height, 1, s.width, s.height);
            bfl.flobPath = "dim";
            dfl.flobPath = "dim";
            ...
            into.add(new LineDecor(new int[]{wh, hh, wh, s.height, wh, hh, s.width, hh}, 8, Color.red, 1));
        }
    }
}
