...
public class UndoList {
     ...
     *
     * @see UndoList
     * @see org.gzigzag.storage.ZZCacheDimension
     */
    public interface Op {
        /**
         * Undo the operation.
    ...
    public int stamp() {
        ...
        if (stamps[nstamps - 1]==nops) return stamp0 + nstamps - 1;
        makeStampRoom(1);
        stamps[nstamps++] = nops;
    ...
    public int commit() {
        ...
        for (int i = nops - 1; i >= 0; i--) {
            if (ops[i] instanceof Op) ((Op) ops[i]).commit(ops, i);
        }
