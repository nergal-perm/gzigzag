...
public class ZZWindows {
    ...
    private static ZZObs obs = new ZZObs() {
        public void chg() {
            update();
        }
    };
    ...
    public static Rectangle getBounds(ZZCell c) {
        return getBounds(c, null);
    }

    public static void setBounds(ZZCell c, Rectangle r) {
        Rectangle b = getBounds(c);
        if (b!=null && b.equals(r)) return;
        ZZUtil.putInts(c.s("d.1", -1), "d.bounds", false, new int[]{r.x, r.y, r.width, r.height});
    }
    ...
    public static synchronized ZZViewComponent getContent(ZZCell c) {
        ...
        if (txt.equals("Canvas")) {
            ZZCanvasView v;
            if (views.get(c) instanceof ZZCanvasView) v = (ZZCanvasView) views.get(c);
            else views.put(c, v = new ZZCanvasView());
    ...
    private static void updateWindowAttributes(ZZCell c, Container w) {
        ...
        if (col!=null) {
            ...
            if (Character.isDigit(colText.charAt(0)) || colText.charAt(0)=='-')
