...
public class Dumper {
    ...
    static public void read(ZZCell hdl, Reader reader) {
        ...
        if (r.cells.length > 0) hdl.connect("d.dump-conn", 1, r.cells[0]); // save start cell!
        else ZZLogger.log("Strange: Dump without cells...");
    }
    ...
    static public ZZModule module = new ZZModule() {
        public void action(String id, ZZCell code, ZZCell target, ZZView view, ZZView cview, String key, Point pt, ZZScene xi) {
            if (id.equals("Dump")) write(target, new OutputStreamWriter(System.out));
