...
public class Dump {
     ...
     */
    public static void writeDump(ZZCell[] cells, String[] dims, Hashtable ids, int nextid, String title, Writer writer) {
        ...
        for (int i = 0; i < cells.length; i++) {
            ...
            if (c.getSpan()!=null)
                ZZLogger.log("ATTENTION! Span converted to plain text in Dump " + "at cell " + c);
        }
        ...
        for (int i = 0; i < dims.length; i++) {
            ...
            for (int j = 0; j < cells.length; j++) {
                ZZCell neg = cells[j];
                ZZCell pos = neg.s(d);
                if (pos!=null && ids.get(pos)!=null) w.println(id(neg, ids) + " " + id(pos, ids));
            }
        }
    ...
    public static class Read {
        ...
        private final String[] split(BufferedReader r, String sp) throws IOException {
            String s = r.readLine();
            if (s.equals("")) return null;
            int i = s.indexOf(sp);
            return new String[]{s.substring(0, i), s.substring(i + sp.length())};
        }
        ...
        public Read(ZZSpace sp, Reader reader) {
            try {
                ...
                while ((parts = split(r, ": "))!=null) {
                    ...
                    else if (var.equals("Next free ID")) nextid = Integer.parseInt(val);
                    else if (var.equals("Title")) title = val;
                ...
                while (!(s = r.readLine()).equals("")) {
                    ...
                    while ((parts = split(r, " "))!=null) {
                        ...
                        if (c1==null || c2==null)
                            throw new ZZError("Dump input: attempt to connect a " + "cell whose content isn't specified " + "(" + (c1==null ? var:val) + ")");
                        c1.connect(s, 1, c2);
                    }
                }
