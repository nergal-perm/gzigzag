...
public class ZZUpdateManager implements Runnable {
    ...
    public static void addView(ZZView v) {
        ordering.addElement(v);
    }

    public static void rmView(ZZView v) {
    ...
    public static void thaw() {
        ...
        synchronized (ordering) {
            if (disabled <= 0) throw new ZZError("thaw() without matching freeze()!");
    ...
    private void timeNewCycle() {
        ...
        else if (millis < defmillis - inctime) millis += inctime;
    ...
    private float timeAnimFract() {
        ...
        if (millis > 20) fract = (cur - lastStart) / (double) millis;
