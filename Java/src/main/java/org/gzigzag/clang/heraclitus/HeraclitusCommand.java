...
class HeraclitusCommand extends ZZCommand implements ZOb {
    ...
    protected void init__zob() {
        if (op==null) throw new ZZError("Must specify Heraclitus Clang operation!");
    }
    ...
    public void execCallback(ZZCell target, ZZView view, ZZView cview, String key, Point pt, ZZScene xi) {
        ZZCell curspar = op.N();
        ZZCell curs1 = op.N();
        ...
        try {
            p("Enter heraclitus: ");
            HeraclitusClang.execute(op, curspar);
            p("Exit heraclitus");
        } finally {
