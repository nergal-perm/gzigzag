...
public class AllPrimitives {
    ...
    static private final void pa(String s) {
    ...
    static void setZaubertrankText(ZZCell main, Object[] parts) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            else if (parts[i] instanceof String) c.N("d.zt-text", 1).setText((String) parts[i]);
            else throw new ZZError("Strange type at " + i + " in " + parts + ": " + parts[i]);
            c = c.s("d.zt-text", 1);
        }
    }
     ...
     */
    public static final String[] standardPrimitives = {"+", "*", "<", "<=", "==", "!=", ">=", ">", ":=", "block", "while", "print", "STEP", "END", "terminate",};
    ...
    static abstract class BinaryOp extends Primitive {
        ...
        public void install(ZZCell main) {
            ...
            setType(left, type);
            setType(right, type);
            left.setText("(" + id + " left)");
            right.setText("(" + id + " right)");
    ...
    public static Primitive block = new Primitive() {
        ...
        public ZZCell evaluate(Expression exp, Namespace context) {
            ...
            ZZCell result = null;
            while (c!=null) {
                if (c.equals(exp.main)) throw new SyntaxError("Looping Archimedes block!");
    ...
    public static Primitive _while = new Primitive() {
        public void install(ZZCell main) {
            ...
            setType(cond, "boolean");
            setType(act, "what");
            cond.setText("(while condition)");
            act.setText("(while action)");
            setZaubertrankText(main, new Object[]{"while(", cond, ") do ", act});
        }
    ...
    public static Primitive print = new Primitive() {
        public void install(ZZCell main) {
            ZZCell par = main.N("d.1");
            par.setText("(print what)");
            setType(par, "what");
    ...
    public static Primitive terminate = new Primitive() {
        public void install(ZZCell main) {
            ZZCell par = main.N("d.1");
            setType(par, "(which cell?)");
            par.setText("(early termination cell)");
            setZaubertrankText(main, new Object[]{"Terminate early, using ", par, " as the early termination cell."});
        }
    ...
    abstract static class PathOp extends Primitive {
        ...
        public abstract void install(ZZCell main, ZZCell cpar, ZZCell dimpar, ZZCell dirpar);
        ...
        public ZZCell evaluate(Expression exp, Namespace context) {
            ...
            if (dircell!=null) dir = Data.i(Archimedes.getValue(dircell, context));

            ZZCell par = null;
            ZZCell parcell = exp.main.s("d.1", 3);
            if (parcell!=null) par = Archimedes.getValue(parcell, context);
    ...
    public static PathOp step = new PathOp() {
        public void install(ZZCell main, ZZCell cpar, ZZCell dimpar, ZZCell dirpar) {
            setZaubertrankText(main, new Object[]{cpar, ": Step ", dirpar, " on ", dimpar, "."});
        }
    ...
    public static PathOp end = new PathOp() {
        public void install(ZZCell main, ZZCell cpar, ZZCell dimpar, ZZCell dirpar) {
            setZaubertrankText(main, new Object[]{cpar, ": Go to the endcell ", dirpar, " on ", dimpar});
        }
    ...
    public static Primitive incparam = new Primitive() {
        public void install(ZZCell main) {
            main.N("d.1");
            setZaubertrankText(main, new Object[]{"incparam (for testing " + "purposes only!)"});
        }
