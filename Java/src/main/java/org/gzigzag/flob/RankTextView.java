/* DO NOT EDIT THIS FILE. THIS FILE WAS GENERATED FROM flob/RankTextView.zob,
...
import org.gzigzag.media.SimpleLineBreaker;
import org.gzigzag.media.SimpleLineInfo;
...
public class RankTextView implements FlobView, ZOb {
     ...
     * <p>Default value: <PRE> new ScalableFont("SansSerif", Font.PLAIN, 16);</PRE>.
     *
     * @structparam 1
     */

    public ScalableFont font = new ScalableFont("SansSerif", Font.PLAIN, 16);
    /**
     ...
     * <p>Default value: <PRE> "d.2";</PRE>.
     *
     * @structparam 1
     */

    public String dim = "d.2";
    /**
    ...
    private int readParams(ZZCell start, int mask) {
        ...
        while (n!=null) {
            ...
            if (s.equals("width")) {
            ...
            } else if (s.equals("bulge")) {
                mask |= 4;
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    bulge = Integer.parseInt(s);
                } catch (Exception e) {
                    ZZLogger.exc(e);
                }
            } else if (s.equals("cuts")) {
                mask |= 8;
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    if (s.equals("true")) cuts = true;
                    else if (s.equals("false")) cuts = false;
                    else { /* XXX ??? */ }
            ...
            } else if (s.equals("billow")) {
                mask |= 32;
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    if (s.equals("true")) billow = true;
                    else if (s.equals("false")) billow = false;
                    else { /* XXX ??? */ }
                ...
                try {
                    ZZCell c = n.s("d.1");
                    s = c.getText();
                    if (s.equals("true")) xor = true;
                    else if (s.equals("false")) xor = false;
                    else { /* XXX ??? */ }
    ...
    class LayBillow implements SimpleLineInfo.Layout {
        ...
        public final int getWidth(int line) {
            ...
            float bfunc = (float) (1.0 / (1 + (line * (float) line) / (bulge * bulge)));
     ...
     */
    public Rectangle raster(FlobSet into, ZZCell c, int offs, boolean doBillow, int startx, int starty, int xormode) {
        ...
        boolean first = true;
        for (ZZCell h = head; h!=null && (first || h!=head); h = h.s(dim, 1)) {
            first = false;
            if (h==c) soffs = offs + s.length();
            s.append(h.getText());
        }
        if (soffs < 0) throw new ZZError("??? ARGH");
        String txt = s.toString();
        SimpleLineInfo sli = new SimpleLineInfo(txt, soffs, (doBillow ? layb:layf));
        ...
        for (ZZCell h = head; h!=null && (first || h!=head); h = h.s(dim, 1)) {
            ...
            if (h.s("d.cut", 1)!=null) {
                // There was a cut.
                into.add(new LineDecor(curx, cury, curx, cury + fm.getHeight(), Color.red, 1));
            }

            while (curoffs + len - nth > linestops[curline + 1]) {
                p("Parent: " + nth + " " + len + " " + curline + " " + linestops[curline + 1]);
                SplitCellFlob1 cur = new SplitCellFlob1(curx, cury, 100, fm.stringWidth(st.substring(nth, nth + (linestops[curline + 1] - curoffs))), fm.getHeight(), h, f, fm, parent, nth, linestops[curline + 1] - curoffs, xormode, bgcolor);
                if (bgcolor==null) cur.showCursor();
                cur.setPrev(latest);
                latest = cur;
                if (curs!=ZZCursorReal.NO_OFFSET) addCurs(into, cur, curs);
                parent = cur;
                into.add(cur);
            ...
            if (curline >= linestops.length - 1) break;

            p("Last: " + nth + " " + len + " " + curline + " " + linestops[curline + 1] + " " + st.length());
            int w = 0;
            String substr = st.substring(nth, len);
            p("Substr: " + substr);
            SplitCellFlob1 last = new SplitCellFlob1(curx, cury, 100, (w = fm.stringWidth(substr)), fm.getHeight(), h, f, fm, parent, nth, len - nth, xormode, bgcolor);
            if (bgcolor==null) last.showCursor();
            last.setPrev(latest);
            latest = last;

            if (curs!=ZZCursorReal.NO_OFFSET) addCurs(into, last, curs);
            curoffs += len - nth;
            curx += w;
            into.add(last);
        }
    ...
    public Rectangle raster(FlobSet into, ZZCell c, int offs, boolean doBillow, int startx, int starty) {
        return raster(into, c, offs, doBillow, startx, starty, SplitCellFlob1.XOR_NO);
    }

    private void addCurs(FlobSet into, SplitCellFlob1 sf, int ind) {
        if (!(ind >= sf.start && ind <= sf.start + sf.n)) return;
        int x = sf.getStrX(ind);
        if (x >= 0) into.add(new LineDecor(x, sf.y, x, sf.y + sf.h, Color.black, sf.d));
    }
